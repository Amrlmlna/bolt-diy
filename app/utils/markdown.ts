  ...
  return (tree: any) => {
    visit(tree, (node: any) => {
      if (node.type === 'html' && node.value && node.value.startsWith('<think>')) {
        ...
        node.value = `<div class="__bitThought__">${cleanedContent}`;
...
const rehypeSanitizeOptions: RehypeSanitizeOptions = {
  ...
  attributes: {
    ...
    div: [
      ...
      'data*',
      ['className', '__bitArtifact__', '__bitThought__', '__bitQuickAction', '__bitSelectedElement__'],

      // ['className', '__bitThought__']
    ],
    button: [
      ...
      'name',
      'value',
      ['className', '__bitArtifact__', '__bitThought__', '__bitQuickAction'],
    ],
  },
  strip: [],
};
