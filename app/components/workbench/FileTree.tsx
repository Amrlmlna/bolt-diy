...
function ContextMenuItem({ onSelect, children }: { onSelect?: () => void; children: ReactNode }) {
  return (
    <ContextMenu.Item
      onSelect={onSelect}
      className="flex items-center gap-2 px-2 py-1.5 outline-0 text-sm text-bit-elements-textPrimary cursor-pointer ws-nowrap text-bit-elements-item-contentDefault hover:text-bit-elements-item-contentActive hover:bg-bit-elements-item-backgroundActive rounded-md"
    >
...
function InlineInput({ depth, placeholder, initialValue = '', onSubmit, onCancel }: InlineInputProps) {
  ...
  return (
    <div
      className="flex items-center w-full px-2 bg-bit-elements-background-depth-4 border border-bit-elements-item-contentAccent py-0.5 text-bit-elements-textPrimary"
      style={{ paddingLeft: `${6 + depth * NODE_PADDING_LEFT}px` }}
    >
      <div className="scale-120 shrink-0 i-ph:file-plus text-bit-elements-textTertiary" />
      <input
        ref={inputRef}
        type="text"
        className="ml-2 flex-1 bg-transparent border-none outline-none py-0.5 text-sm text-bit-elements-textPrimary placeholder:text-bit-elements-textTertiary min-w-0"
    ...
    <>
      <ContextMenu.Root>
        <ContextMenu.Trigger>
          <div
            ...
            className={classNames('relative', {
              'bg-bit-elements-background-depth-2 border border-dashed border-bit-elements-item-contentAccent rounded-md':
                isDragging,
            })}
          >
            {children}
        ...
        <ContextMenu.Portal>
          <ContextMenu.Content
            style={{ zIndex: 998 }}
            className="border border-bit-elements-borderColor rounded-md z-context-menu bg-bit-elements-background-depth-1 dark:bg-bit-elements-background-depth-2 data-[state=open]:animate-in animate-duration-100 data-[state=open]:fade-in-0 data-[state=open]:zoom-in-98 w-56"
          >
            <ContextMenu.Group className="p-1 border-b-px border-solid border-bit-elements-borderColor">
            ...
            <ContextMenu.Group className="p-1 border-t-px border-solid border-bit-elements-borderColor">
              {!isFolder ? (
                <>
    ...
    <FileContextMenu onCopyPath={onCopyPath} onCopyRelativePath={onCopyRelativePath} fullPath={folder.fullPath}>
      <NodeButton
        className={classNames('group', {
          'bg-transparent text-bit-elements-item-contentDefault hover:text-bit-elements-item-contentActive hover:bg-bit-elements-item-backgroundActive':
            !selected,
          'bg-bit-elements-item-backgroundAccent text-bit-elements-item-contentAccent': selected,
        })}
        depth={folder.depth}
...
function File({
  ...
  return (
    <FileContextMenu onCopyPath={onCopyPath} onCopyRelativePath={onCopyRelativePath} fullPath={fullPath}>
      <NodeButton
        className={classNames('group', {
          'bg-transparent hover:bg-bit-elements-item-backgroundActive text-bit-elements-item-contentDefault': !selected,
          'bg-bit-elements-item-backgroundAccent text-bit-elements-item-contentAccent': selected,
        })}
        depth={depth}
        iconClasses={classNames('i-ph:file-duotone scale-98', {
          'group-hover:text-bit-elements-item-contentActive': !selected,
        })}
        onClick={onClick}
      >
        <div
