...
export const DialogTitle = memo(({ className, children, ...props }: RadixDialog.DialogTitleProps) => {
  return (
    <RadixDialog.Title
      className={classNames('text-lg font-medium text-bit-elements-textPrimary flex items-center gap-2', className)}
      {...props}
    >
      {children}
    </RadixDialog.Title>
...
export const DialogDescription = memo(({ className, children, ...props }: RadixDialog.DialogDescriptionProps) => {
  return (
    <RadixDialog.Description
      className={classNames('text-sm text-bit-elements-textSecondary mt-1', className)}
      {...props}
    >
      {children}
    </RadixDialog.Description>
  ...
  return (
    <RadixDialog.Portal>
      ...
      <RadixDialog.Content asChild>
        <motion.div
          className={classNames(
            'fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white dark:bg-gray-950 rounded-lg shadow-xl border border-bit-elements-borderColor z-[9999] w-[520px] focus:outline-none',
            className,
          )}
          initial="closed"
          animate="open"
          ...
          <div className="flex flex-col">
            ...
            {showCloseButton && (
              <RadixDialog.Close asChild onClick={onClose}>
                <IconButton
                  icon="i-ph:x"
                  className="absolute top-3 right-3 text-bit-elements-textTertiary hover:text-bit-elements-textSecondary"
                />
              </RadixDialog.Close>
            )}
          </div>
            ...
            <Button
              ...
              className={
                variant === 'destructive'
                  ...
                  : 'bg-bit-elements-item-backgroundAccent text-bit-elements-item-contentAccent hover:bg-bit-elements-button-primary-backgroundHover'
              }
            >
              {isLoading ? (
                <>
    ...
    return (
      <div
        ...
        className={classNames(
          ...
          selectedItems.includes(item.id)
            ? 'bg-bit-elements-item-backgroundAccent'
            : 'bg-bit-elements-bg-depth-2 hover:bg-bit-elements-item-backgroundActive',
        )}
        style={{
          ...style,
          width: '100%',
      ...
      >
        ...
        <div className="grid gap-1.5 leading-none">
          ...
          {item.description && <p className="text-xs text-bit-elements-textSecondary">{item.description}</p>}
        </div>
      </div>
    );
  };
      ...
      <Dialog showCloseButton={false}>
        <div className="p-6 bg-white dark:bg-gray-950 relative z-10">
          ...
          <DialogDescription className="mt-2 mb-4">
            ...
            <span className="text-bit-elements-item-contentAccent font-medium">{confirmLabel}</span>.
          </DialogDescription>

          <div className="py-4">
            <div className="flex items-center justify-between mb-4">
              <span className="text-sm font-medium text-bit-elements-textSecondary">
              ...
              <Button
                ...
                className="text-xs h-8 px-2 text-bit-elements-textPrimary hover:text-bit-elements-item-contentAccent hover:bg-bit-elements-item-backgroundAccent bg-bit-elements-bg-depth-2 dark:bg-transparent"
              >
            ...
            <div
              className="pr-2 border rounded-md border-bit-elements-borderColor bg-bit-elements-bg-depth-2"
              style={{
                maxHeight,
              }}
            >
              {items.length > 0 ? (
                <FixedSizeList
                  ...
                  itemSize={60}
                  className="scrollbar-thin scrollbar-thumb-rounded scrollbar-thumb-bit-elements-bg-depth-3"
                >
                  {ItemRenderer}
                </FixedSizeList>
              ) : (
                <div className="text-center py-4 text-sm text-bit-elements-textTertiary">No items to display</div>
              )}
            </div>
          </div>
          ...
          <div className="flex justify-between mt-6">
            <Button
              variant="outline"
              onClick={onClose}
              className="border-bit-elements-borderColor text-bit-elements-textPrimary hover:bg-bit-elements-item-backgroundActive"
            >
              Cancel
            </Button>
            <Button
