  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className="max-w-chat rounded-lg border-l-2 border-l-[#098F5F] border border-bit-elements-borderColor bg-bit-elements-background-depth-2"
      >
        {/* Header */}
        ...
        <div className="px-4">
          {!isConnected ? (
            <div className="p-3 rounded-md bg-bit-elements-background-depth-3">
              <span className="text-sm text-bit-elements-textPrimary">
            ...
            <>
              <div
                className="flex items-center p-2 rounded-md bg-bit-elements-background-depth-3 cursor-pointer"
                onClick={() => setIsCollapsed(!isCollapsed)}
              >
                <div className="i-ph:database text-bit-elements-textPrimary mr-2"></div>
                <span className="text-sm text-bit-elements-textPrimary flex-grow">
                  {description || 'Create table and setup auth'}
                </span>
                <div
                  className={`i-ph:caret-up text-bit-elements-textPrimary transition-transform ${isCollapsed ? 'rotate-180' : ''}`}
                ></div>
              </div>

              {!isCollapsed && content && (
                <div className="mt-2 p-3 bg-bit-elements-background-depth-4 rounded-md overflow-auto max-h-60 font-mono text-xs text-bit-elements-textSecondary">
        ...
        {/* Message and Actions */}
        <div className="p-4">
          <p className="text-sm text-bit-elements-textSecondary mb-4">{message}</p>
