...
import { createFilesContext, extractCurrentContext, extractPropertiesFromMessage, simplifyBitActions } from './utils';
...
export async function selectContext(props: {
  ...
  const processedMessages = messages.map((message) => {
    if (message.role === 'user') {
      ...
      content = simplifyBitActions(content);

      content = content.replace(/<div class=\\"__bitThought__\\">.*?<\/div>/s, '');
...
---
  ...
  if (totalFiles == 0) {
    throw new Error(`Bit failed to select files`);
  }
