...
export class ImportExportService {
    ...
    try {
      ...
      return {
        ...
        core: {
          ...
          bit_user_profile: this._safeGetItem('bit_user_profile'),
          bit_settings: this._safeGetItem('bit_settings'),
          bit_profile: this._safeGetItem('bit_profile'),
        ...
        features: {
          // Feature flags
          viewed_features: this._safeGetItem('bit_viewed_features'),
          developer_mode: this._safeGetItem('bit_developer_mode'),
        ...
        ui: {
          // Tab configuration
          bit_tab_configuration: this._safeGetItem('bit_tab_configuration'),
        ...
        debug: {
          ...
          acknowledged_debug_issues: this._safeGetItem('bit_acknowledged_debug_issues'),
          acknowledged_connection_issue: this._safeGetItem('bit_acknowledged_connection_issue'),

          // Error logs
          error_logs: this._safeGetItem('error_logs'),
          bit_read_logs: this._safeGetItem('bit_read_logs'),
        ...
        updates: {
          ...
          last_acknowledged_update: this._safeGetItem('bit_last_acknowledged_version'),
        },
  ...
  static async deleteAllChats(db: IDBDatabase): Promise<void> {
    ...
    localStorage.removeItem('bit_chat_history');
  ...
  private static async _importComprehensiveFormat(data: any): Promise<void> {
    ...
    if (data.ui) {
      ...
      if (data.ui.bit_tab_configuration) {
        try {
          this._safeSetItem('bit_tab_configuration', data.ui.bit_tab_configuration);
        } catch (err) {
    ...
    if (data.debug) {
      ...
      const debugLocalStorageKeys = [
        'bit_acknowledged_debug_issues',
        'bit_acknowledged_connection_issue',
        'error_logs',
        'bit_read_logs',
      ];
    ...
    if (data.updates) {
      ...
      if (data.updates.last_acknowledged_update) {
        try {
          this._safeSetItem('bit_last_acknowledged_version', data.updates.last_acknowledged_update);
        } catch (err) {
